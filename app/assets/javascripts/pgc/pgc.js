...
function cleanFormErrorState(frm) {
...
function cleanFormInputs(frm) {
  frm.find("input[type=text]").val('');
}

function showValidationErrors(dialog, xhr) {
  var errors,
      errorText;
...
function refreshTreeButtonsState(category, childCategories, childComptes) {
  if (category == 2) {
  ...
  $.ajax({
    ...
    success: function(data) {
      tag.html(data).dialog({
        ...
        modal: true,
        close: function() {
          cleanFormErrorState(tag.find('form'));
          cleanFormInputs(tag.find('form'));
        }
      }).dialog('open');
    }
  });
...
function fireOptionJs() {
  ...
  /* Create jqxExpander */
  $("#pgcIndex").jqxExpander({showArrow: false, toggleMode: 'none' });
  ...
  pgcTree.bind('select', function (event) { 
    ...
    $.ajax({
      ...
      data: 'id='+item.id,
      success: function(data) {
        refreshTreeButtonsState(data["category"], data["childCategories"], data["childComptes"]);
      },
      error: function(data) {
